function price = BinaryCallImplicit(Q,S,K,r,T,sigma,q,Smax,dS,N)
M = round(Smax/dS);
dt = T/N;
...
Expire = N+1:-1:1; % Vector to calculate time.
binary_payoff = sign(StockGrid - K); % Returns 1 if binary payoff, else returns 0 or -1.
FiniteMesh(:,N+1)=max(Q*binary_payoff,0); % @ Expiration
FiniteMesh(M+1,:)=Q*exp(-r*dt*Expire); % @ S_max, deep in the money.
FiniteMesh(1,:)=0; % @ S=0
...
for j = N: -1 : 1
	...
	adjust(M-1) = -c(M-1)*FiniteMesh(M+1,j); %equals zero for put
%   A\B = INV(A)*B
